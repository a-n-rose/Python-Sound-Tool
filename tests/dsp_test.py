...
def test_scalesound_min_minus100_max100():
    np.random.seed(0)
    input_samples = np.random.random_sample((5,))
    output_samples = sp.dsp.scalesound(input_samples, min_val=-100, max_val=100)
...
def test_scalesound_min_minuspoint25_maxpoint25():
    np.random.seed(0)
    input_samples = np.random.random_sample((5,))
    output_samples = sp.dsp.scalesound(input_samples, min_val=-.25, max_val=.25)
...
def test_normalize_default():
    np.random.seed(0)
    input_samples = np.random.random_sample((5,))
    output_samples = sp.feats.normalize(input_samples)
...
def test_resample_audio_sr100_to_80():
    ...
    test_audio, sr1 = sp.dsp.generate_sound(freq=10, sr=100,dur_sec=0.05)
    test_resampled, sr2 = sp.dsp.resample_audio(test_audio, 
                                             sr_original=sr1,
                                             sr_desired=80)
...
def test_stereo2mono_mono_input():
    ...
    data_mono = sp.dsp.stereo2mono(data)
    assert np.array_equal(data, data_mono)

def test_apply_sample_length():
    data = np.array([1,2,3,4,5])
    num_samps = 10
    new_data = sp.dsp.apply_sample_length(data, num_samps)
...
def test_apply_sample_length_mirrored():
    data = np.array([1,2,3,4,5])
    num_samps = 10
    new_data = sp.dsp.apply_sample_length(data, num_samps, mirror_sound=True)
...
def test_apply_sample_length_stereo_longer():
    ...
    num_samps = 5
    new_data = sp.dsp.apply_sample_length(data,num_samps, clip_at_zero=False)
    assert len(new_data) == num_samps
...
def test_apply_num_channels_1d_to_3d():
    ...
    data3d = sp.dsp.apply_num_channels(data, 3)
...
def test_apply_num_channels_3d_to_2d():
    ...
    data2d = sp.dsp.apply_num_channels(data, 2)
    expected = np.array([[1, 1],[1, 1],[1, 1],[1, 1]])
    assert np.array_equal(expected, data2d)
    
def test_zeropad_sound_mono_targetlen8():
    data = np.array([1,2,3,4])
    data_zeropadded = sp.dsp.zeropad_sound(data, sr=4, target_len=8)
...
def test_zeropad_sound_mono_targetlen8_delay1sec():
    data = np.array([1,2,3,4])
    data_zeropadded = sp.dsp.zeropad_sound(data, sr=4, target_len=8, delay_sec=1)
...
def test_zeropad_stereo_targetlen_5():
    ...
    data[:,0] = np.array([0,1,2])
    data[:,1] = np.array([1,2,3])
    data_zeropadded = sp.dsp.zeropad_sound(data, sr=3, target_len=5)
...
def test_random_selection_samples_wrap():
    ...
    got = sp.dsp.random_selection_samples(x, len_section_samps = 7, 
