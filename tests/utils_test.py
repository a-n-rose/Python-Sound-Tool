...
def test_path_or_samples_str():
    item_type = pyso.utils.path_or_samples(test_audiofile)
    assert item_type == 'path'
    
def test_path_or_samples_pathlib():
    item_type = pyso.utils.path_or_samples(pathlib.Path(test_audiofile))
    assert item_type == 'path'
    
def test_path_or_samples_tuple_librosa():
    item = librosa.load(test_audiofile)
    item_type = pyso.utils.path_or_samples(item)
...
def test_path_or_samples_str_not_real_path():
    ...
    with pytest.raises(ValueError):
        item_type = pyso.utils.path_or_samples('blah')
    
def test_path_or_samples_pathlib_not_real_path():
    print('IF TEST FAILES: For now, function does not test for path validity.')
    with pytest.raises(ValueError):
        item_type = pyso.utils.path_or_samples(pathlib.Path('blah'))
    
def test_match_dtype_float2int():
    array_original = np.array([1,2,3,4])
    array_to_change = np.array([1.,2.,3.,4.,5.])
    array_adjusted = pyso.utils.match_dtype(array_to_change, array_original)
...
def test_check_dir_default_create():
    ...
    test_dir = pyso.utils.check_dir(test_dir)
...
def test_check_dir_check_exists():
    test_dir = './testtesttest/'
    test_dir = pyso.utils.check_dir(test_dir, make=True)
    test_dir = pyso.utils.check_dir(test_dir, make=False)
    assert isinstance(test_dir, pathlib.PosixPath)
    assert os.path.exists(test_dir)
    os.rmdir(test_dir)
...
def test_check_dir_check_exists_notwriteinto_raiseerror():
    ...
    with pytest.raises(FileExistsError):
        test_dir = pyso.utils.check_dir(test_dir, make=False, append=False)
    os.rmdir(test_dir)
...
def test_string2list():
    audiofiles = pyso.files.collect_audiofiles(audio_dir,wav_only=False,
                                         recursive=False)
    audiofiles_string = str(audiofiles)
    audiofiles_checked = pyso.utils.restore_dictvalue(audiofiles_string)
...
def test_string2list_loaddict():
    ...
    d_path = pyso.utils.save_dict(
        dict2save = d, 
        filename = test_dict_path)
    d_loaded = pyso.utils.load_dict(d_path)
