...
class Generator:
    def __init__(self, data_matrix1, data_matrix2=None, feature_type=None, 
        ...
        Parameters
        ----------
        data_matrix1 : np.ndarray [size=(num_samples, batch_size, num_frames, num_features)]
        ...
        self.batch_size = 1
        self.samples_per_epoch = data_noisy.shape[0]
        ...
        self.batches_per_sample = datax.shape[1]
        self.num_frames = datax.shape[2]
        if self.datay is None:
            # assumes last column of features is the label column
            self.num_feats = datax.shape[3] - 1 
            # separate the label from the feature data
            self.datay = datax[:,:,:,-1]
            # remove label data from feature data
            self.datax = datax[:,:,:,:-1]
            self.labels = True
        else:
            self.labels = False
    ...
    def generator(self):
        while 1:
            ...
            if self.adjust_shape is not None:
                # expects data to have shape (batches_per_sample, num_frames, num_feats)
                if isinstance(self.adjust_shape, int): 
                    # adjust just the number of features:
                    desired_shape = (self.batches_per_sample,
                                     self.num_frames,
                                     self.adjust_shape)
                elif isinstance(self.adjust_shape, tuple):
                    # adjust the entire shape (for example for narrowband and 
                    # wideband data)
                    desired_shape = self.adjust_shape
                else:
                    raise TypeError('Expected `adjust_shape` to be type int or tuple, '+\
                        'not type {}.'.format(type(zeropadding)))
                batch_x = pyst.data.adjust_data_shape(batch_x, desired_shape)
                
                if not self.labels:
                    batch_y = pyst.data.adjust_data_shape(batch_y, desired_shape)
