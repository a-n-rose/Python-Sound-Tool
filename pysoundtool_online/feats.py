...
def plotsound(audiodata, feature_type='fbank', win_size_ms = 20,
              ...
              sr=None, save_pic=False, name4pic=None, energy_scale='power_to_db',
              mono=None, use_scipy = True, **kwargs):
    ...
    **kwargs : additional keyword arguments
    ...
    feats = pyso.feats.get_feats(audiodata, feature_type=feature_type, 
                      ...
                      sr=sr, mono = mono, use_scipy = use_scipy)
...
def get_feats(sound,
    ...
    **kwargs : additional keyword arguments
    ...
    if isinstance(sound, str) or isinstance(sound, pathlib.PosixPath):
        if mono is None:
            mono = True
        data, sr = pyso.loadsound(sound, sr=sr, dur_sec = dur_sec,
                                  mono = mono, use_scipy = use_scipy)
    ...
    if 'signal' not in feature_type:
        if rate_of_change or rate_of_acceleration:
            d, d_d = pyso.feats.get_change_acceleration_rate(feats, use_scipy = use_scipy)
            if rate_of_change:
...
def get_change_acceleration_rate(spectro_data, use_scipy = True):
    ...
    if use_scipy:
        delta = librosa_delta(spectro_data)
        delta_delta = librosa_delta(spectro_data,order=2)
    else:
...
def librosa_delta(data, width=9, order=1, axis=-1, mode='interp', **kwargs):
    ...
    References
    ----------
    librosa.feature.utils.delta
        This function is pulled from librosa's source code; the online version
        of PySoundTool does not import librosa due to conflicts with online 
        environments and the use of soundfile.
    '''
